class Monitor {
  ...
  boolean fadeOut;
  boolean dissappear;
  boolean sync = true; // for sync with clock
  ...
  void display() {
    ...
    //scaling info
    if (scaling) {
      hwInfo(xpos, xpos + w_display, ypos, ypos + h_display, max(10, min(20, floor(w_display / 20))));
    }
  ...
  void speedInfo() {
    // textSize(20);
    textSize(max(10, w_display / 20));
    fill(255);
    pushMatrix();
    translate( xpos + w_display / 2, ypos - floor(w_display / 20));
  ...
  void dateInfo() {
    ...
    fill(255);
    pushMatrix();
    translate( xpos , ypos + h_display + floor(w_display / 20));
